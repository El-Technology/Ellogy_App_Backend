...
public interface IUsecasesService
{
    ...
    /// <param name="userIdFromToken">User id taken from jwt token</param>
    /// <returns>A task representing the asynchronous operation. The paginated list of use cases 
    /// with detailed information.</returns>
    Task<PaginationResponseDto<UsecaseFullDto>> GetUsecasesAsync(
    ...
    /// <returns>A task representing the asynchronous operation. The paginated list of use cases
    /// with detailed information.</returns>
    Task<UsecaseFullDto> UpdateUsecaseAsync(
        Guid usecaseId, UsecaseDataFullDto usecase, Guid userIdFromToken);
}
